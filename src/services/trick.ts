import type { Card, Trick, PlayedCard, Suit } from '@/models/types'
import { compareCards, getEffectiveSuit } from './deck'
...
/**
 * Count tricks won by a specific player
 */
export function countTricksWonByPlayer(tricks: Trick[], playerId: number): number {
  let playerTricks = 0
...
/**
 * Get the next player to lead (winner of previous trick)
 */
export function getNextLeader(trick: Trick): number {
  if (trick.winnerId === null) {
    throw new Error('Cannot get next leader from incomplete trick')
  }
