import type { Card, Suit, Trump, Bid } from '@/models/types'
import { BidAction, Suit as SuitEnum } from '@/models/types'
import { getSameColorSuit } from './deck'
 ...
 * Round 1: everyone has had a chance
 * Round 2: goes until someone calls or dealer is forced to call
 */
export function isBiddingComplete(
  ...
  if (biddingRound === 1) {
    // Round 1 ends when everyone has had a chance (4 passes)
    return passCount === 4
  } else {
    // Round 2: dealer must call (stick the dealer rule)
    // So it ends when we get back to dealer OR someone calls
