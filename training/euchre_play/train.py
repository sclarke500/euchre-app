...
def masked_accuracy(
    clf: MLPClassifier, X: np.ndarray, y: np.ndarray, legal: np.ndarray
) -> float:
    """Argmax over legal cards using decision_function / predict_proba."""
    if hasattr(clf, "predict_proba"):
        proba = clf.predict_proba(X)
        # Map class indices to columns
...
def main() -> None:
    ...
    # Also raw sklearn accuracy (unmasked)
    raw_val = float(clf.score(Xva, yva))

    metrics = {
        "train_examples": st_tr["examples"],
