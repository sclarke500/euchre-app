 ...
 * ONNX/in-process inference is the future ship path — not this.
 */
...
import { euchreActionKey } from '@67cards/shared'
...
/** Hybrid: hard bids/discard, model plays cards. */
export async function choosePlayModel(
  ...
  const observation = encodeEuchreObservation(ctx.state, ctx.seat, ctx.legal)
  try {
    const action = await bridge.predict(observation, ctx.legal)
    if (!ctx.legal.some(a => euchreActionKey(a) === euchreActionKey(action))) {
