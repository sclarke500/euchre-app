...
const cardStyle = computed(() => {
  ...
  if (stage.value === 'start') {
    ...
    // Stack offset: cards lower in deck are slightly lower visually
    const stackOffset = (props.totalCards - props.dealOrder) * 0.5
    return {
  ...
  if (stage.value === 'flying' || stage.value === 'landed') {
    ...
    return {
      ...
      opacity: 1,
      transition: 'all 1s cubic-bezier(0.25, 0.46, 0.45, 0.94)',  // Slow for debugging
...
onMounted(() => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      stage.value = 'landed'
      props.onComplete()
    }, 1100)  // Slightly longer than 1s transition
  }, props.delay)
})
</script>
